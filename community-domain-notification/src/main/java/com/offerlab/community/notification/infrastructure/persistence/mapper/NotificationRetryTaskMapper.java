...
@Mapper
public interface NotificationRetryTaskMapper extends BaseMapper<NotificationRetryTaskPO> {
    ...
    @Insert("""
            INSERT INTO t_notif_retry_task (
                ...
                content_json, task_status, retry_count, next_retry_time, last_error
            ) VALUES (
                #{id}, #{dedupKey}, #{scene}, #{receiverUid}, #{senderUid}, #{notifType}, #{targetType}, #{targetId},
                #{contentJson}, #{taskStatus}, #{retryCount}, #{nextRetryTime}, #{lastError}
            )
            ON DUPLICATE KEY UPDATE
                task_status = IF(task_status = 1, task_status, 0),
                next_retry_time = IF(task_status = 1, next_retry_time, VALUES(next_retry_time)),
                last_error = VALUES(last_error),
                update_time = NOW(3)
            """)
    int upsertPending(NotificationRetryTaskPO task);
    ...
    @Select("""
            ...
            LIMIT #{limit}
            """)
    List<NotificationRetryTaskPO> findClaimed(@Param("owner") String owner, @Param("limit") int limit);
    ...
    @Select("SELECT task_status AS status, COUNT(*) AS count FROM t_notif_retry_task GROUP BY task_status")
