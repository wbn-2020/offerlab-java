...
VALUES
    ...
    (990310000000000003, @demo_uid, 990200000000000003, 'learning', 0, 'EXPLAIN 字段要讲 type/key/rows/Extra，再落到索引顺序。', 'expression', '慢 SQL 排查会先用慢日志定位，再看 EXPLAIN 和数据分布，最后用组合索引、覆盖索引或分页改写验证效果。', NULL, DATE_ADD(NOW(3), INTERVAL 1 DAY), NOW(3) - INTERVAL 3 DAY, 1, 2, NOW(3) - INTERVAL 6 DAY, NOW(3) - INTERVAL 1 DAY),
    (990310000000000004, @demo_uid, 990200000000000004, 'review', 1, 'Kafka 堆积要按 lag、消费耗时、失败重试、分区倾斜和下游耗时拆。', 'project', '幂等用业务唯一键和数据库唯一约束；堆积先确认 lag 和处理耗时，再判断扩容、限流还是下游降级。', 'S: 促销消息堆积导致履约延迟；T: 恢复消费并避免重复扣减；A: 增加幂等表、调整批量提交、隔离慢下游；R: lag 在 20 分钟内恢复到安全水位。', NOW(3) - INTERVAL 1 HOUR, NOW(3) - INTERVAL 2 DAY, 2, 3, NOW(3) - INTERVAL 5 DAY, NOW(3) - INTERVAL 1 HOUR),
    ...
    (990310000000000006, @demo_uid, 990200000000000006, 'learning', 1, 'JVM 题要把 dump、GC 日志和对象引用链连起来。', 'concept', '我会先看 Full GC 后老年代回收比例，再抓 heap dump，用 MAT 看 dominator tree 和 GC Roots，确认是否是缓存或静态集合持有。', NULL, DATE_ADD(NOW(3), INTERVAL 2 DAY), NOW(3) - INTERVAL 1 DAY, 1, 2, NOW(3) - INTERVAL 2 DAY, NOW(3) - INTERVAL 1 DAY),
    (990310000000000007, @demo_uid, 990200000000000007, 'todo', 0, '准备把 OfferLab 题库搜索作为项目案例来讲。', NULL, '搜索接口会用 ES 做关键词召回，DB 查询详情；索引失败进入重试任务，前端展示任务状态和降级结果。', 'S: 题库增长后搜索延迟升高；T: 保持搜索体验；A: 引入索引重建和失败重试；R: 热门关键词响应稳定在百毫秒级。', NULL, NULL, 0, 1, NOW(3) - INTERVAL 2 DAY, NOW(3)),
    (990310000000000008, @demo_uid, 990200000000000008, 'review', 1, 'STAR 表达要量化结果，不要只说做了优化。', 'expression', '我会用场景、任务、行动、结果四段回答，并把结果落到错误率、P99 延迟和排查效率。', 'S: 核心接口在活动期间超时；T: 我负责定位并降低超时；A: 加 tracing、优化慢 SQL、热点缓存预热；R: P99 从 1.8s 降到 420ms，错误率低于 0.1%。', NOW(3) - INTERVAL 30 MINUTE, NOW(3) - INTERVAL 1 DAY, 1, 2, NOW(3) - INTERVAL 2 DAY, NOW(3) - INTERVAL 30 MINUTE)
ON DUPLICATE KEY UPDATE
    uid = VALUES(uid),
...
VALUES
    (990400000000000001, @demo_uid, '深测科技', 'Java 后端', 'medium', 'Kafka', 3, 3, 12, 1280, 'completed', NOW(3) - INTERVAL 1 DAY, NOW(3) - INTERVAL 1 HOUR)
ON DUPLICATE KEY UPDATE
    uid = VALUES(uid),
    company = VALUES(company),
    position = VALUES(position),
...
VALUES
    (990410000000000001, 990400000000000001, @demo_uid, 990200000000000002, 1, 'Redis 缓存和数据库双写不一致时，你会如何设计更新策略？', '旁路缓存、删除失败补偿和热点重建。', '深测科技', 'Java 后端', '一面', 'medium', '先更新数据库再删除缓存，删除失败写入重试任务；热点 key 使用逻辑过期和互斥重建。', '还需要补充为什么不直接先写缓存。', 4),
    (990410000000000002, 990400000000000001, @demo_uid, 990200000000000004, 2, 'Kafka 消费端如何保证幂等？如果出现消息堆积你会先看哪些指标？', '幂等键、唯一约束、lag、分区倾斜。', '深测科技', '后端工程师', '二面', 'hard', '用业务唯一键和去重表保证幂等；堆积先看 lag、消费耗时、失败重试比例、分区倾斜和下游耗时。', '回答完整，但 STAR 案例可以更紧。', 4),
    (990410000000000003, 990400000000000001, @demo_uid, 990200000000000008, 3, '项目经历里你如何用 STAR 讲清一次稳定性优化？', 'STAR 四段和量化结果。', '深测科技', 'Java 后端', '技术加面', 'easy', '我会用场景、任务、行动、结果四段讲，把 P99、错误率和恢复时间作为结果指标。', '表达更顺了，下一版要加个人贡献边界。', 4)
ON DUPLICATE KEY UPDATE
    uid = VALUES(uid),
