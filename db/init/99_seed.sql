...
VALUES
    (
        ...
        'mastered',
        1,
        '同类方法调用不走代理，要主动说出事务边界为什么放。service 编排层。',
        NULL,
        '我会先解。Spring AOP 代理机制，再结合订单结算项目说明如何把事务入口放到应用服务层，并补充 rollbackFor 和传播行为。',
    ...
    (
        ...
        'expression',
        '。SQL 排查会先用慢日志定位，再。EXPLAIN 和数据分布，最后用组合索引、覆盖索引或分页改写验证效果。',
        NULL,
    ...
    (
        ...
        'S: 促销消息堆积导致履约延迟；T: 恢复消费并避免重复扣减；A: 增加幂等表、调整批量提交、隔离慢下游；R: lag 。20 分钟内恢复到安全水位。',
    ...
    (
        ...
        'learning',
        1,
        'JVM 题要。dump、GC 日志和对象引用链连起来。',
        'concept',
        '我会先看 Full GC 后老年代回收比例，再抓 heap dump，用 MAT 。dominator tree 。GC Roots，确认是否是缓存或静态集合持有。',
        NULL,
    ...
    (
        ...
        'S: 核心接口在活动期间超时；T: 我负责定位并降低超时；A: 。tracing、优化慢 SQL、热点缓存预热；R: P99 。1.8s 降到 420ms，错误率低于 0.1%。',
...
VALUES
    (
        ...
        '一。',
        'medium',
        '先更新数据库再删除缓存，删除失败写入重试任务；热。key 使用逻辑过期和互斥重建。',
        '还需要补充为什么不直接先写缓存。',
        4,
        1,
        'SUCCEEDED',
